...
**Owning agent:** `shared/agents/spring-onboarding/AGENT.md`
...
- `shared/maven/parent-pom-fragment.xml` and `shared/skills/maven-harness-pom/SKILL.md` (authoritative reference).
- `shared/skills/jacoco-coverage-policy/SKILL.md`, `shared/skills/pit-mutation-tuning/SKILL.md`, `shared/skills/flyway-or-liquibase-detection/SKILL.md`.
...
- `.specs/_baseline.json` (refreshed via `scripts/harness.sh --baseline` after the wiring run).
- `.specs/_stack.json` (refreshed via `scripts/detect-stack.sh`).
...
6. **Document deferrals.** For each layer deferred in step 2, write `.specs/adr/ADR-NNN-<slug>.md` per `shared/skills/adr-authoring/SKILL.md` (Context, Decision drivers, Considered options, Decision outcome with rationale, Consequences, Trigger to revisit). Add a corresponding `DEBT-NNN` entry to `.specs/_known-debt.md`.
7. **Verify.** Run `mvn -pl <module> -am verify`. The build must be **green**. Then run `scripts/harness.sh --module <module> --baseline` and capture the result into `.specs/_baseline.json`.
8. **Refresh artifacts.** Re-run `scripts/detect-stack.sh <module>/pom.xml > .specs/_stack.json`. Update `.specs/_known-debt.md` to mark the harness-layers debt resolved (preserve the entry, prepend the resolution note). Update `.specs/_starter-design.md` with the chosen profiles, code style, and test-naming conventions.
...
- `scripts/harness.sh --module <module> --report` shows every wired layer as `pass` (or `skipped` if it lives in an opt-in profile).
