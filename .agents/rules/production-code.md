...
trigger: glob
globs: ["src/main/**/*.java"]
description: "Production code rule — TDD precondition + Spring Boot 4 conventions."
---
...
Hard convention: **package by feature/domain, not by layer**. Top-level packages are bounded contexts (e.g. `giftcard`, `order`), each with `api/dto/`, `api/exception/`, `model/`, `repository/`, `service/` sub-packages. Never create top-level `controller`, `service`, `repository`, `model`, `dto`, or `util` packages.
